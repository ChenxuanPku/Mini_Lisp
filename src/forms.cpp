    ...
    if (auto name = args[0]->asSymbol()) {
        ...
        if (args[0]->isPair()) {
            if (auto name = args[0]->toHead()->asSymbol()) {
                ...
                env.Push_Back(*name, std::make_shared<LambdaValue>(
                                         para, body, env.shared_from_this()));
            }
        }
    }
...
ValuePtr ifForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    ...
    if (ans->isBoolean())
        if (!ans->asBool()) return std::move(env.eval(args[2]));
...
ValuePtr andForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    ...
    for (int i{0}; i != args.size(); i++) {
        ...
        if (ans->isBoolean())
            if (!ans->asBool()) return std::make_shared<BooleanValue>(false);
...
ValuePtr condForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    for (int i{0}; i != args.size(); i++) {
        ...
        if (son[0]->toString() == "else") {
            if (i != args.size() - 1)
                throw LispError("Else should be the last one.");
...
ValuePtr letForm(const std::vector<ValuePtr>& args, EvalEnv& env) {
    ...
    if (args.size() < 2)
        throw LispError(
            "The number of parameters provided does not meet the "
            "requirements.");
    for (int i{1}; i != args.size(); i++) process.push_back(std::move(args[i]));
    auto Lambda =
        std::make_shared<LambdaValue>(newArgs, process, env.shared_from_this());
